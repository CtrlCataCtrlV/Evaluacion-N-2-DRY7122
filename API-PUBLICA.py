...
def get_directions():
    ...
    if json_status == 0:
        ...
        distance = "{:.2f}".format(json_data["route"]["distance"])
        ...
        fuel_required = float(distance) / mileage
...
def open_directions_window(orig, dest, duration, distance, fuel_required, maneuvers, vehicle_name):
    ...
    fuel_label = tk.Label(directions_window, text="Combustible Requerido: " + "{:.2f}".format(fuel_required) + " litros")
    fuel_label.pack()
